...
export default class Graph extends React.Component<IGraphProps,
  { showComponent: Boolean, clickElement : string }> {
  ...
  public _onButtonOpen1 = ev => {
    ...
    this.setState({ showComponent: true, clickElement: elmnt.getAttribute('data-element') });
  }
...
this.setState({ showComponent: true, clickElement: elmnt.getAttribute('data-element') });
  ...
  public _onButtonClose = ev => {
    this.setState({ showComponent: false, clickElement: null });
    }
  ...
  public handleOnMouseOut5 = e =>{
    document.getElementById('path5').style.fillOpacity='1';
  }
      ...
      if (that.state.clickElement == "1") {
        return (<div className={styles.box} style={{ left: 0}}>
          <div className={styles.row}>
            <h2 className="ms-Grid-col ms-sm6 ms-md8 ms-lg10">{escape(that.props.windowTitle1)}</h2>
            <div className="ms-Grid-col ms-sm6 ms-md4 ms-lg2" onClick={that._onButtonClose}>
              <span className={styles.close}>X</span>
            </div>
          </div>
          <p className={styles.bottomContent}>{escape(that.props.windowDescription1)}</p>
          <a href={escape(that.props.windowLink1)}>More informations</a>
        </div>
        );
      } else  if (that.state.clickElement == "2") {
          return (<div className={styles.box} style={{ left: 0}}>
            <div className={styles.row}>
              <h2 className="ms-Grid-col ms-sm6 ms-md8 ms-lg10">{escape(that.props.windowTitle2)}</h2>
              <div className="ms-Grid-col ms-sm6 ms-md4 ms-lg2" onClick={that._onButtonClose}>
                <span className={styles.close}>X</span>
              </div>
            </div>
            <p className={styles.bottomContent}>{escape(that.props.windowDescription2)}</p>
            <a href={escape(that.props.windowLink2)}>More informations</a>
          </div>
          );
        } else   if (that.state.clickElement == "3") {
            return (<div className={styles.box} style={{ right: 0}}>
              <div className={styles.row}>
                <h2 className="ms-Grid-col ms-sm6 ms-md8 ms-lg10">{escape(that.props.windowTitle3)}</h2>
                <div className="ms-Grid-col ms-sm6 ms-md4 ms-lg2" onClick={that._onButtonClose}>
                  <span className={styles.close}>X</span>
                </div>
              </div>
              <p className={styles.bottomContent}>{escape(that.props.windowDescription3)}</p>
              <a href={escape(that.props.windowLink3)}>More informations</a>
            </div>
            );
          } else   if (that.state.clickElement == "4") {
              return (<div className={styles.box} style={{ right: 0}}>
                <div className={styles.row}>
                  <h2 className="ms-Grid-col ms-sm6 ms-md8 ms-lg10">{escape(that.props.windowTitle4)}</h2>
                  <div className="ms-Grid-col ms-sm6 ms-md4 ms-lg2" onClick={that._onButtonClose}>
                    <span className={styles.close}>X</span>
                  </div>
                </div>
                <p className={styles.bottomContent}>{escape(that.props.windowDescription4)}</p>
                <a href={escape(that.props.windowLink4)}>More informations</a>
              </div>
              );
            } else   if (that.state.clickElement == "5") {
                return (<div className={styles.box} style={{ right: 0}}>
                  <div className={styles.row}>
                    <h2 className="ms-Grid-col ms-sm6 ms-md8 ms-lg10">{escape(that.props.windowTitle5)}</h2>
                    <div className="ms-Grid-col ms-sm6 ms-md4 ms-lg2" onClick={that._onButtonClose}>
                      <span className={styles.close}>X</span>
                    </div>
                  </div>
                  <p className={styles.bottomContent}>{escape(that.props.windowDescription5)}</p>
                  <a href={escape(that.props.windowLink5)}>More informations</a>
                </div>
                );
              } else   if (that.state.clickElement == "6") {
                  return (<div className={styles.box} style={{ right: 0}}>
                    <div className={styles.row}>
                      <h2 className="ms-Grid-col ms-sm6 ms-md8 ms-lg10">{escape(that.props.windowTitle6)}</h2>
                      <div className="ms-Grid-col ms-sm6 ms-md4 ms-lg2" onClick={that._onButtonClose}>
                        <span className={styles.close}>X</span>
                      </div>
                    </div>
                    <p className={styles.bottomContent}>{escape(that.props.windowDescription6)}</p>
                    <a href={escape(that.props.windowLink1)}>More informations</a>
                  </div>
                  );
                } else   if (that.state.clickElement == "7") {
                    return (<div className={styles.box} style={{ left: 0}}>
                      <div className={styles.row}>
                        <h2 className="ms-Grid-col ms-sm6 ms-md8 ms-lg10">{escape(that.props.windowTitle7)}</h2>
                        <div className="ms-Grid-col ms-sm6 ms-md4 ms-lg2" onClick={that._onButtonClose}>
                          <span className={styles.close}>X</span>
                        </div>
                      </div>
                      <p className={styles.bottomContent}>{escape(that.props.windowDescription7)}</p>
                      <a href={escape(that.props.windowLink7)}>More informations</a>
                    </div>
                    );
                  } else   if (that.state.clickElement == "8") {
                      return (<div className={styles.box} style={{ left: 0}}>
                        <div className={styles.row}>
                          <h2 className="ms-Grid-col ms-sm6 ms-md8 ms-lg10">{escape(that.props.windowTitle8)}</h2>
                          <div className="ms-Grid-col ms-sm6 ms-md4 ms-lg2" onClick={that._onButtonClose}>
                            <span className={styles.close}>X</span>
                          </div>
                        </div>
                        <p className={styles.bottomContent}>{escape(that.props.windowDescription8)}</p>
                        <a href={escape(that.props.windowLink8)}>More informations</a>
                      </div>
                      );
                    }
